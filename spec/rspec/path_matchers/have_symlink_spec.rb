...
RSpec.describe 'the have_symlink matcher' do
  ...
  describe 'checking existance' do
    subject { expect(tmpdir).to have_symlink(entry_name) }
  ...
  describe 'not_to have_symlink' do
    subject { expect(tmpdir).not_to have_symlink(entry_name) }
    ...
    context 'when given other options' do
      subject { expect(tmpdir).not_to have_symlink(entry_name, owner: 'user') }

      before { File.symlink('target', path) }
